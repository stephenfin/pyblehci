	...
	def _build_command(self, cmd, **kwargs):
		...
		for field in packet_structure:
			...
			#data wasn't given
				...
				if field_len is not None:
					...
					if default_value:
						...
						#otherwise fail
						raise KeyError("The expected field of {0} of length " +
							"{1} was not provided".format(field_name, field_len))
			...
			if field_len and len(field_data) != field_len:
				raise ValueError("The data provided for '{0}' was not {1} " +
					"bytes long").format(field_name, field_len)
